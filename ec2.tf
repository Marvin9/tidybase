...
resource "aws_launch_template" "tidybase" {
  depends_on = [
...
resource "aws_instance" "tidybase" {
  ...
  launch_template {
    id = aws_launch_template.tidybase.id
  }

  tags = {
    Name = "tidybase-compute"
  }
}

output "instance" {
  value = "http://${aws_instance.tidybase.public_dns}"
}
